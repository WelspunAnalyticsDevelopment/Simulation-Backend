...
router.get('/', (req, res) => {
  ...
  jsonData.map(({key,value}, index) => {
    ...
    query+= `${key} as ${value} from SALESPLAN_SIMULATION WHERE [PRODUCT_CAT] = '${req.query.selectedProduct}' AND (MERCHANT like '%AVINASH%' OR TEAM_LEADER like '%AVINASH MALANI%')`
    }
  })  
  console.log(query);
  ...
  if(req.query.productName == 'Terry Towel'){
    ...
    query += `
      ...
      FROM   salesplan_publish AS sp
            INNER JOIN SALESPLAN_SIMULATION AS ss
                    ON sp.[unique_identification_no] = ss.[unique_identification_no]
                        AND sp.[product_cat] = 'Rugs' AND (sp.[VERSION_NO] like '%Avinash_Malani%' OR sp.[TEAM_LEADER] like '%AVINASH MALANI%')
      `;
  }
    ...
    query += `
      ...
      ss.[APRIL_(TOTAL_KGS)] AS 'APRIL (TOTAL KGS)',
      ss.[APRIL_(SQ_MTR)] AS 'AvPRIL (SQ MTR)',
      sp.[APRIL_KGS] AS 'APRIL KGS',
      ss.[APRIL(TUFTING_KGS)] AS 'APRIL(TUFTING KGS)',
      ss.[APR_MTR] AS 'APR MTR',
      sp.[APR_RATE_AS_PER_CURRENCY] AS 'APR RATE AS PER CURRENCY',
